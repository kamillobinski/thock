...
struct ModeDatabase {
    private var modeStorage: [Brand: [Author: [Mode]]] = {
        ...
        let userModes = ModeDatabase.loadCustomModes()
        
        if !userModes.isEmpty {
            base[.Custom, default: [:]][.custom] = userModes
        }
    ...
    func getAllBrands() -> [Brand] {
        var keys = Array(modeStorage.keys)
        
        // Check if .Custom has any modes under any author
        if let customAuthors = modeStorage[.Custom] {
            let allCustomModes = customAuthors.values.flatMap { $0 }
            if allCustomModes.isEmpty {
                keys.removeAll { $0 == .Custom }
            }
        } else {
            keys.removeAll { $0 == .Custom }
        }
    ...
    static func loadCustomModes() -> [Mode] {
        ...
        do {
            let subdirectories = try FileManager.default.contentsOfDirectory(at: basePath, includingPropertiesForKeys: nil)
                .filter { $0.hasDirectoryPath }
            
            for folderURL in subdirectories {
                ...
                let metadata = try JSONDecoder().decode(ModeMetadata.self, from: data)
                
                let mode = Mode(
                    id: metadata.id,
                    name: metadata.name,
                    isNew: metadata.isNew,
                    path: "CustomSounds/\(folderURL.lastPathComponent)/"
                )
                
                print("Loaded custom mode: \(mode.name)")
                modes.append(mode)
            }
        } catch {
            print("Error loading user modes: \(error)")
        }
